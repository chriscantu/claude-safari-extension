...
> **Critical:** `screenshotService` must NOT have an inline `= ScreenshotService()` initializer when a designated `init` also assigns it — Swift will give a compile error. Remove the inline initializer before adding the `init`.
...
In `pollForExtensionResponse`, replace the block that removes pending entries and calls `deliverExtensionResponse`:
...
Replace the existing `handleScreenshotAction` with:
...
Note: `tabId` is now `Int` (with `-1` sentinel). `screenshotService.captureScreenshot(tabId:)` accepts `Int?`; Swift auto-promotes `Int` to `Int?`.
...
In the Phase 6 table, change `gif_creator` status from `📋` to `✅`:

```
| `gif_creator` — record, stop, export animated GIFs | [017](Specs/017-gif-creator.md) | ✅ |
```

Add after the Phase 6 table:
